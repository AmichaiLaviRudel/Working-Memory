...
def main(parent_dir: str = None, progress_fn=None, status_fn=None):
    ...
    acx_before_window = (float(acx_time_axis[0]), 0.0)
    acx_after_window = (0.0, 0.6)

    action_units_acx, action_t_acx, action_p_acx, acx_action_aligned = find_action_modulated_units(
    ...
    print(f"Found {len(action_units_acx)} action-modulated ACx units (lick aligned)")
    ...
    save_dataframe_to_csv(
        acx_units_df,
        os.path.join(results_dir, "tables", "acx_all_units_metrics.csv"),
        "ACx comprehensive unit metrics",
    )
    ...
    # Action- and outcome-aligned PSTH for modulated ACx units
    if acx_action_aligned is not None and len(action_units_acx) > 0:
        save_raw_psth_for_active_units(
            acx_action_aligned,
            action_units_acx,
            action_p_acx,
    ...
    ofc_before_window = (float(ofc_time_axis[0]), 0.0)
    ofc_after_window = (0.0, 0.6)

    action_units_ofc, action_t_ofc, action_p_ofc, ofc_action_aligned = find_action_modulated_units(
    ...
    # Action- and outcome-aligned PSTH for modulated OFC units
    if ofc_action_aligned is not None and len(action_units_ofc) > 0:
        save_raw_psth_for_active_units(
            ofc_action_aligned,
            action_units_ofc,
            action_p_ofc,
    ...
    if len(active_units_ofc) > 0:
        ...
        save_dataframe_to_csv(
            ofc_units_df,
            os.path.join(results_dir, "tables", "ofc_all_units_metrics.csv"),
            "OFC comprehensive unit metrics",
        )
    ...
    set_progress(100, f"Analysis complete! Results saved to: {results_dir}")
