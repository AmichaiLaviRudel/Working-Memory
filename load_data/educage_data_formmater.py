...
def run_educage_formatter(source_path: str, out_csv: str) -> str:
    """Run the Educage data formatter on `source_path` and save to `out_csv`.

    This is a direct refactor of the original script body so it can be
    called from Streamlit or the CLI. It returns the output CSV path.
    """
    path1 = source_path
    ...
    if "licks_time" in df.columns:
        ...
        if "licks_time_rd" in df.columns:
            ...
            # licks_time_rd comes before licks_time
        ...
        if "start_dt" in df.columns:
            ...
            licks_exploded = (
                licks_exploded.dropna(subset=["licks_time_list"]) if not licks_exploded.empty else licks_exploded
            )
            if not licks_exploded.empty:
                licks_exploded["lick_dt"] = pd.to_datetime(
                    licks_exploded["date"]
                    + " "
                    + licks_exploded["licks_time_list"].astype(str),
                    errors="coerce",
                )
                ...
                first_rt = licks_exploded.groupby("trial_id")["rel_sec"].min().mul(1000.0)
    ...
    grouped = (
        df_sorted.groupby(["mouse_id", "date", "level"], as_index=False)
        .agg(
            ...
            if "go_no_go_norm" in df_sorted.columns
            else ("go_no_go", list),
            ...
            if "score_norm" in df_sorted.columns
            else ("score", list)
            ...
            if "rt_first_ms" in df_sorted.columns
            else ("stim_value", lambda s: [pd.NA] * len(s)),
            licks=(
                "licks_rel",
                list,
            )
            if "licks_rel" in df_sorted.columns
            else ("stim_value", lambda s: [None] * len(s)),
            start_time=(
                "start_time",
                list,
            )
            if "start_time" in df_sorted.columns
            else ("stim_value", lambda s: [pd.NA] * len(s)),
        )
    ...
    def _parse_level_tones(level: Any) -> int:
        try:
    ...
    def _parse_level_boundaries(level: Any) -> int:
        try:
    ...
    grouped_out = (
        grouped_out.assign(_sort_dt=_sort_dt)
        .sort_values(["_sort_dt", "MouseName", "Notes"])
        .drop(columns=["_sort_dt"])
    )

    # Format Stimuli lists without commas, space-separated within brackets
    def _format_space_list(lst: Any) -> str:
        try:
            vals = []
            for v in lst:
    ...
    _export_df.to_csv(out_csv_path, index=False)
    print(f"Grouped summary saved to: {out_csv_path}")
...
def main() -> None:
    ...
    If two CLI args are given, they are used as source_path and out_csv.
    Otherwise, fall back to the original hard-coded paths.
    """
    if len(sys.argv) >= 3:
        source_path = sys.argv[1]
        out_csv = sys.argv[2]
    else:
        # Original defaults to preserve existing behaviour
        source_path = r"Z:\Shared\Noam\results\Educage_headbar_pilot_15_12_2025\Educage_headbar_pilot_15_12_2025.txt"
        out_csv = r"Z:\\Shared\\Amichai\\Code\\DB\\users_data\\Amichai\\Educage_headbar_pilot_experimental_data.csv"

    run_educage_formatter(source_path, out_csv)
